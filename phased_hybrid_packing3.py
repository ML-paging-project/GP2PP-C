...
def find_next_box3(t, k, miss_cost, phase, models,
    ...
    if phase == 4:
        ...
        if int(t.box_start_pointer / check_length) > t.check_point or phase_change:
            ...
            if (t.run_oracle or phase_change) and \
                    t.memory_impact / t.det_req2mi[t.box_start_pointer - 1] > 2:
    ...
    while len(finished.keys()) != len(trace_names):
        while len(packed_traces.keys()) + len(finished) < len(trace_names):
            ...
            if test_traces[next_trace].box_end_pointer < len(test_traces[next_trace].seq):
                test_traces[next_trace] = find_next_box3(test_traces[next_trace], k, s,
        ...
        if phase_change:
            ...
            for name in trace_names:
                if name not in finished.keys():
                    ...
                    if phase < 4:
                        _, _, test_traces[name].det_req2box, test_traces[
                            name].det_req2counter, r2m = deterministic_algorithm(test_traces[name].seq,
                                                                                 k, number_of_box_kinds, s)
                        for j in range(test_traces[name].box_start_pointer,
                                       len(test_traces[name].seq)):
                            test_traces[name].det_req2mi[j] = r2m[j] - r2m[test_traces[name].box_start_pointer - 1] + \
                                                              test_traces[name].det_req2mi[
                                                                  test_traces[name].box_start_pointer - 1]
                    test_traces[name] = find_next_box3(test_traces[name], k, s, phase, models, number_of_box_kinds,
                                                       window, phase_change)
